...
object ConnectionExt {
  implicit class ConnectionExtImplicit(val conn: Connection) extends AnyVal {
...
trait ConnectionExtImplicits {
  @inline implicit def wrapConnectionToConnectionExt(conn: Connection): ConnectionExt.ConnectionExtImplicit =
    new ConnectionExt.ConnectionExtImplicit(conn)
}
