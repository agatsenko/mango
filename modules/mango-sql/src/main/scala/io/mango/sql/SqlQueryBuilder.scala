...
class SqlQueryBuilder(val sc: StringContext) extends AnyVal {
...
trait SqlQueryBuilderImplicits {
  @inline implicit def wrapStringContextToSqlQueryBuilder(sc: StringContext): SqlQueryBuilder = new SqlQueryBuilder(sc)
}
