...
class ManagedResource3Tests extends FunSuite
                                    with BaseManagedResourceTests
                                    with TableDrivenPropertyChecks
                                    with MockFactory
                                    with Matchers {
  type AC = AutoCloseable
  type TManagedResource = ManagedResource3[TestResource, TestResource, TestResource]
  type TActionResult = Int
  type TAction = (TestResource, TestResource, TestResource) => TActionResult
  ...
  test("using(managedResource)(action) should close all managed resources after action is successful completed") {
    ...
    val actualActionResultOpt = using(managedRes)(action(_, _, _))
  ...
  test("using(managedResource)(action) should close all managed resources, even if resources throw exception") {
    ...
    intercept[Throwable](using(managedRes)(action(_, _, _)))
    assertAllResourcesClosed(managedRes)
  }
  ...
  test("using(managedResource)(action) should close all managed resources after action is unsuccessful completed") {
    ...
    val actualActionResult = intercept[TestException](using(managedRes)(action(_, _, _)))
  ...
  private def newActionMock(managedRes: TManagedResource, result: TActionResult): TAction = {
    val action = mockFunction[TestResource, TestResource, TestResource, TActionResult]
    action.expects(managedRes.r1, managedRes.r2, managedRes.r3).returns(result)
    action
  }

  private def newActionMock(managedRes: TManagedResource, result: Throwable): TAction = {
    val action = mockFunction[TestResource, TestResource, TestResource, TActionResult]
    action.expects(managedRes.r1, managedRes.r2, managedRes.r3).throws(result)
    action
  }
}
